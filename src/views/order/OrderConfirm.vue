<template>
  <div class="order-confirm">
    ...
    <div class="wrapper">
      ...
      <!-- 添加收货地址对话框 -->
      <el-dialog :title="diglogTitle" :visible.sync="diglogVisable" width="39%" @click="diglogClose" :close-on-click-model="false">
        <!-- 内容主体区域 -->
        <el-form :v-model="addrForm" :rules="formRules" ref="addrFormRef" label-width="80px">
          <el-form-item label="用户名" prop="username">
            <el-input v-model="addrForm.username" style="width: 217px;" placeholder="姓名"></el-input>
          </el-form-item>
          <el-form-item label="手机号码" prop="mobile">
            <el-input v-model="addrForm.mobile" style="width: 217px;" placeholder="请填写正确的手机号码"></el-input>
          </el-form-item>
          <el-form-item label="所在地区" prop="addrZone">
            <el-cascader v-model="addrForm.addrZone" :options="options" :props="{ expandTrigger: 'hover' }" @change="handeleChange" clearable></el-cascader>
          </el-form-item>
          <el-form-item label="详细地址" prop="addrDetail">
            <el-input type="textarea" v-model="addrForm.addrDetail" style="width: 217px;" placeholder="街道、小区。楼牌号等，无需重复填写省市区"></el-input>
          </el-form-item>
          <!-- 底部按钮区域 -->
          <div class="addr-foot">
...
<script>
  ...
  import { getAddrList,getCartList,addAddress,updAddress,delAddress,addOrder } from 'network/request'
  export default {
    name:'order-confirm',
    data () {
      // 验证手机号的规则
    ...
    methods: {
      ...
      async addrOnSubmit() {
        ...
        this.$refs.addrFormRef.validate(async valid => {
          if(!vaild) return
          let { id, usename, mobile, addrZone, addrDetail } = this.addrForm
          let newAddr = {
            id,
            username,
            mobile,
